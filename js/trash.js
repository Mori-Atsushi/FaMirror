var trash = function() {
	...
	$('#item_trash').click( function() {
		trash_data = user_data[user_id]['setting'][trash_id]['config'];

		var flag = user_data[user_id]['setting'][trash_id]['notification'] == '1';
		$('#trash_notification').prop('checked', flag);
...
/*		if(weather_data['prefecture'] != null) {
		...
		$('#setting_trash').animate({'left': '0%'}, speed);
		$('#detail').animate({'left': '-100%'}, speed);
	});

	//データを送信して戻る
	$('#trash_back').click( function() {
/*		var d;
		var prefecture, area;
		...
		$('#setting_trash').animate({'left': '100%'}, speed);
